...
const AboutSection: React.FC = () => (
  <div id="about" style={{ padding: '40px 40px 48px', maxWidth: 740, borderBottom: '2px solid #c0c0c0' }}>
    <h2 style={{
      fontSize: 48,
    ...
    <div style={{ marginBottom: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap' }}>
        <span style={{ fontSize: 21, fontWeight: 'bold', fontFamily: '"Playfair Display", Georgia, serif' }}>
  ...
  <div style={{ marginBottom: 0, paddingBottom: 28, borderBottom: '1px solid #ccc' }}>
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 4, marginBottom: 10 }}>
      ...
      <div style={{ textAlign: 'right', flexShrink: 0 }}>
        {url && (
          <div>
            <a href={url} target="_blank" rel="noreferrer" style={{ fontSize: 15, color: '#000080', textDecoration: 'underline' }}>{url.replace('https://', '')}</a>
          </div>
        )}
    ...
    {description && (
      <p style={{ fontSize: 19, lineHeight: 1.8, marginBottom: 10 }}>{description}</p>
    )}

    <ul style={{ paddingLeft: 20 }}>
      {bullets.map((b, i) => (
        <li key={i} style={{ fontSize: 18, lineHeight: 1.8, marginBottom: 6 }}>{b}</li>
      ))}
    </ul>
  </div>
);
...
const ExperienceSection: React.FC = () => (
  <div id="experience" style={{ padding: '40px 32px 48px', maxWidth: 740, borderBottom: '2px solid #c0c0c0' }}>
    <h2 style={{
      fontSize: 48,
      fontFamily: '"Abril Fatface", Georgia, serif',
      fontWeight: 400,
...
const SkillsSection: React.FC = () => (
  <div id="skills" style={{ padding: '40px 40px 48px', maxWidth: 740 }}>
    <h2 style={{
      fontSize: 48,
      fontFamily: '"Abril Fatface", Georgia, serif',
      fontWeight: 400,
    ...
    <ul style={{ paddingLeft: 20, display: 'flex', flexDirection: 'column', gap: 10 }}>
      {softSkills.map((skill, i) => (
        <li key={i} style={{ fontSize: 18, lineHeight: 1.7, color: '#222' }}>{skill}</li>
      ))}
    </ul>
  </div>
);
...
interface ScrollableSectionsProps {
  scrollTo?: 'about' | 'experience' | 'skills';
}

const ScrollableSections: React.FC<ScrollableSectionsProps> = ({ scrollTo }) => {
